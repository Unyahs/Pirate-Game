...
func _ready():
	...
	player.get_node("AnimationTree").play("Idle")
	...
	for level in levels:
		if level.name in LevelData.level_dic:
			if LevelData.level_dic[level.name]["unlocked"] == true:
				level.get_node("Sprite2D").texture = load("res://WorldMap/EpisodeFifteen/unlocked.png")
				if LevelData.level_dic[level.name]["beaten"] == true:
					level.get_node("Sprite2D").texture = load("res://WorldMap/EpisodeFifteen/beaten.png")
			else:
				level.get_node("Sprite2D").texture = load("res://WorldMap/EpisodeFifteen/locked.png")
...
func _process(delta):
	...
	if Input.is_action_just_pressed("jump"):
		player.get_node("AnimationTree").play("Select")
	...
	if target_level and target_level.name in LevelData.level_dic and LevelData.level_dic[target_level.name]["unlocked"] and completed_movement:
		...
		player.get_node("AnimationTree").play("Run")
		lerp_progress = 0.0
