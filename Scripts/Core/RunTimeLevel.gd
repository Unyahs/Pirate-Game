...
func set_values():
	for node in get_children():
		...
		if node is FierceTooth or node is Cannon or node is TotemHead1 or node is TotemHead2 or node is TotemHead3 or node is BossTooth:
			max_score += node.score
			max_enemies += 1

func beat_level():
	$"UI Manager/VictoryScreen/FinishLevel".grab_focus()
	$"UI Manager/VictoryScreen/Restart".grab_focus()
